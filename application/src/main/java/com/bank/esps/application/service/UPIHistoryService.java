...
public class UPIHistoryService {
     ...
     * Uses REQUIRES_NEW propagation to avoid holding main transaction lock
     */
     ...
     * 
     * Note: This is only called from coldpath (backdated trades), so performance impact is acceptable
     * Uses REQUIRES_NEW propagation to avoid holding main transaction lock
     */
    @Transactional(propagation = org.springframework.transaction.annotation.Propagation.REQUIRES_NEW)
