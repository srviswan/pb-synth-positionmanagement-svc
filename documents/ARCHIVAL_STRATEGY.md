# Archival Strategy for All Tables

## Overview

All tables in the position management service support **archival** to efficiently move old data to archival storage while keeping active data in the main database. The event store uses **partition-level archival** leveraging PostgreSQL's hash partitioning to archive entire partitions at once. Other tables support row-level archival based on age or business rules.
...
Each table has consistent indexing for archival:

1. **Partial Index on Active Data** (all tables):
   ```sql
   CREATE INDEX idx_<table>_archival_flag ON <table> (archival_flag) 
   WHERE archival_flag = FALSE;
   ```
   - Optimizes queries for active data only
   - Smaller index size

2. **Index on Archived Timestamp** (all tables):
   ```sql
   CREATE INDEX idx_<table>_archived_at ON <table> (archived_at) 
   WHERE archived_at IS NOT NULL;
   ```
   - Supports archival queries and reporting

3. **Composite Index for Event Store** (partition-level archival):
   ```sql
   ...
   ```
   - Optimizes partition-level archival queries
...
### Other Tables: Row-Level Archival

For tables without partitioning (snapshot_store, idempotency_store, reconciliation_breaks, regulatory_submissions, upi_history), use row-level archival:

#### Step 1: Identify Records for Archival

```sql
-- Example: Archive snapshots for TERMINATED positions older than 7 years
SELECT 
    COUNT(*) as ready_for_archival,
    MIN(last_updated_at) as oldest_snapshot
FROM snapshot_store
WHERE archival_flag = FALSE
  AND status = 'TERMINATED'
  AND last_updated_at < CURRENT_TIMESTAMP - INTERVAL '7 years';
```

#### Step 2: Mark Records for Archival

```sql
-- Mark TERMINATED snapshots older than 7 years
...
COPY (
    ...
    WHERE archival_flag = TRUE
      AND archived_at >= CURRENT_DATE - INTERVAL '1 day'
) TO '/archive/snapshot_store_2024.csv' WITH CSV HEADER;
```

#### Similar Process for Other Tables

- **idempotency_store**: Archive by `processed_at` date
- **reconciliation_breaks**: Archive by `resolved_at` date (only RESOLVED breaks)
- **regulatory_submissions**: Archive by `submitted_at` date
- **upi_history**: Archive by `occurred_at` date
...
**Entities with Archival Support:**
- `EventEntity`
- `SnapshotEntity`
- `IdempotencyEntity`
...
### Regulatory Requirements by Table

- **event_store (Trade Events)**: 7 years (typical regulatory requirement)
- **snapshot_store (Position Snapshots)**: 7 years for TERMINATED positions, keep ACTIVE positions
- **idempotency_store**: 1 year (short-term duplicate prevention)
- **reconciliation_breaks**: 5 years (after resolution)
- **regulatory_submissions**: 7 years (regulatory compliance)
- **upi_history**: Permanent (for compliance and audit)
