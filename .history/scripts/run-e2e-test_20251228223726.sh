...
if [ "$HTTP_HIFO_SETUP" = "201" ] || [ "$HTTP_HIFO_SETUP" = "200" ]; then
    ...
    if [ "$LOT_COUNT_HIFO" -ge 3 ]; then
        test_result 0 "HIFO position has multiple lots: $LOT_COUNT_HIFO"
        echo "   Note: HIFO method reduces from highest price first (60.00 > 55.00 > 50.00)"
    else
        test_result 1 "HIFO position should have multiple lots, found: $LOT_COUNT_HIFO"
    fi
else
...
if [ "$HTTP_CONTRACT" = "201" ] || [ "$HTTP_CONTRACT" = "200" ]; then
    ...
    if [ "$(echo "$QTY_CONTRACT >= 200" | bc 2>/dev/null || echo "$((QTY_CONTRACT >= 200))")" = "1" ]; then
        ...
        test_result 1 "Contract-based trade quantity incorrect: $QTY_CONTRACT"
    fi
else
