...
run_concurrent_load() {
    ...
    local min_time=$(echo "$times" | awk 'BEGIN{min=999} {if($1<min) min=$1} END {print min}')
    ...
    local p50=$(echo "$sorted_times" | sed -n "${p50_idx}p" || echo "0")
    ...
    # Return error rate for evaluation
    echo "$error_rate"
}
...
for step in $(seq 1 $RAMP_UP_STEPS); do
    ...
    error_rate=$(run_concurrent_load $current_users 5 "RAMP-STEP${step}")
...
while [ $(date +%s) -lt $end_time ]; do
    ...
    error_rate=$(run_concurrent_load $CONCURRENT_USERS $REQUESTS_PER_USER "SUSTAINED-I${iteration}")
...
error_rate=$(run_concurrent_load $spike_users $REQUESTS_PER_USER "SPIKE")
