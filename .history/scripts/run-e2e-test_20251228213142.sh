...
HTTP_CODE2=$(echo "$TRADE_RESPONSE2" | grep "HTTP_CODE:" | cut -d: -f2 || echo "000")
if [ "$HTTP_CODE2" = "201" ] || [ "$HTTP_CODE2" = "200" ]; then
    ...
    POSITION_RESPONSE2=$(curl -s "${BASE_URL}/api/positions/${POSITION_KEY}" 2>&1)
    if echo "$POSITION_RESPONSE2" | grep -q "openLots"; then
        LOT_COUNT=$(echo "$POSITION_RESPONSE2" | grep -o '"lotId"' | wc -l | tr -d ' ' || echo "0")
        if [ -z "$LOT_COUNT" ]; then
            LOT_COUNT="0"
        fi
