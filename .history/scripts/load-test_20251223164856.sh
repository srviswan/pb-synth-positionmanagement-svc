...
run_concurrent_load() {
    ...
    if [ ! -s "$results_file" ]; then
        ...
        total_requests=$(printf "%d" "$total_requests" 2>/dev/null || echo "0")
        success_count=$(printf "%d" "$success_count" 2>/dev/null || echo "0")
        
        local error_count=$((total_requests - success_count))
    fi
    
    local times=$(grep "^200|" "$results_file" 2>/dev/null | cut -d'|' -f2 || echo "")
    local avg_time=0
    local min_time=0
    local max_time=0
    local p50=0
