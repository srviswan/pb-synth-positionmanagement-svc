...
if [ "$HTTP_INC" = "201" ] || [ "$HTTP_INC" = "200" ]; then
    ...
    if [ "$(echo "$QTY_AFTER > $QTY_BEFORE_INC" | bc 2>/dev/null || echo "0")" = "1" ]; then
        test_result 0 "Position quantity increased: $QTY_BEFORE_INC -> $QTY_AFTER"
    else
        test_result 1 "Position quantity did not increase: $QTY_BEFORE_INC -> $QTY_AFTER"
    fi
else
