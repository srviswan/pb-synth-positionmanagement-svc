...
create_trade() {
    local trade_id=$1
    local account=$2  # Changed: account is now passed directly
    local trade_type=$3
    local qty=$4
    local price=$5
    ...
    local account=${account:-"LOAD-ACC"}
    ...
    for user in $(seq 1 $num_users); do
        (
            ...
            for req in $(seq 1 $requests_per_user); do
                ...
                # Pass account to create_trade function
                result=$(create_trade "$trade_id" "$account" "$trade_type" "$qty" "$price" "$effective_date")
