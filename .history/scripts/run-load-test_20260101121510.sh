...
create_trade() {
    ...
    if [ -n "$iteration" ]; then
        # Vary quantity slightly based on iteration
        local base_qty=$(echo "$json_content" | grep -o '"quantity": [0-9.]*' | cut -d: -f2 | tr -d ' ' || echo "100")
        local new_qty=$((base_qty + (iteration % 50)))
        json_content=$(echo "$json_content" | sed "s/\"quantity\": [0-9.]*/\"quantity\": $new_qty/")
    ...
    for user in $(seq 1 $num_users); do
        (
            ...
            for req in $(seq 1 $requests_per_user); do
                ...
                # Rotate through different trade types
                local json_template=""
                case $((req % 4)) in
                    0)
