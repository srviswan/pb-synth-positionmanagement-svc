...
CREATE TABLE idempotency_store (
    ...
    INDEX idx_trade_id ON idempotency_store (trade_id),
    INDEX idx_processed_at ON idempotency_store (processed_at)
);
...
CREATE TABLE reconciliation_breaks (
    ...
    INDEX idx_position_key ON reconciliation_breaks (position_key),
    INDEX idx_status ON reconciliation_breaks (status),
    INDEX idx_detected_at ON reconciliation_breaks (detected_at)
);
...
CREATE TABLE regulatory_submissions (
    ...
    INDEX idx_trade_id ON regulatory_submissions (trade_id),
    INDEX idx_status ON regulatory_submissions (status),
    INDEX idx_submitted_at ON regulatory_submissions (submitted_at)
);
