...
public class SolaceConfig {
    ...
    public ConnectionFactory solaceConnectionFactory() {
        try {
            ...
            // Solace JMS uses javax.jms.ConnectionFactory, but Spring Boot 3.x uses jakarta.jms.ConnectionFactory
            // Use reflection to set the target connection factory field directly, bypassing method signature
            ...
            // Use reflection to set the targetConnectionFactory field directly
            // This bypasses the method signature that expects jakarta.jms.ConnectionFactory
            try {
                java.lang.reflect.Field targetField = org.springframework.jms.connection.CachingConnectionFactory.class
                    .getDeclaredField("targetConnectionFactory");
                targetField.setAccessible(true);
                targetField.set(cachingFactory, javaxFactory);
                targetField.setAccessible(false);
                log.debug("Set targetConnectionFactory field using reflection");
            } catch (NoSuchFieldException e) {
                // If field doesn't exist, try using the method with reflection
                try {
                    ...
                    setTargetMethod.invoke(cachingFactory, (jakarta.jms.ConnectionFactory) (Object) javaxFactory);
                    log.debug("Set targetConnectionFactory using method reflection");
                } catch (Exception e2) {
                    ...
                    throw new IllegalStateException("Failed to set target connection factory. " +
                            "Cannot bridge javax.jms.ConnectionFactory to jakarta.jms.ConnectionFactory.", e2);
                }
            } catch (Exception e) {
                log.error("Failed to set target connection factory field using reflection", e);
                throw new IllegalStateException("Failed to set target connection factory. " +
                        "Cannot bridge javax.jms.ConnectionFactory to jakarta.jms.ConnectionFactory.", e);
            }
