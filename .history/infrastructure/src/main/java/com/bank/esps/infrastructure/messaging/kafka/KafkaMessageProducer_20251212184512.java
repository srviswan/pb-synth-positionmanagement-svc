...
public class KafkaMessageProducer implements MessageProducer {
    ...
    @Override
    public CompletableFuture<Void> publishRegulatoryEvent(String positionKey, String regulatoryEvent) {
        try {
            String topic = "regulatory-events";
            CompletableFuture<SendResult<String, String>> future = 
                    kafkaTemplate.send(topic, positionKey, regulatoryEvent);
            
            return future.handle((result, ex) -> {
                if (ex == null) {
                    log.info("Published regulatory event for position {} to topic {}", positionKey, topic);
                    return null;
                } else {
                    log.error("Error publishing regulatory event for position {} to topic {}", positionKey, topic, ex);
                    return null; // Don't fail on regulatory event publish errors
                }
            });
            
        } catch (Exception e) {
            log.error("Error publishing regulatory event for position {}", positionKey, e);
            return CompletableFuture.completedFuture(null); // Don't fail on regulatory event publish errors
        }
    }
}
