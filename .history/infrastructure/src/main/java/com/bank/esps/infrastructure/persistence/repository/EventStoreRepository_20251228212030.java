...
public interface EventStoreRepository extends JpaRepository<EventEntity, Long> {
    ...
    List<EventEntity> findByPositionKeyOrderByEventVerAsc(String positionKey);
    
    @Query("SELECT e FROM EventEntity e WHERE e.positionKey = :positionKey AND e.effectiveDate <= :asOfDate ORDER BY e.eventVer ASC")
