...
public class SolaceConfig {
    ...
    public ConnectionFactory solaceConnectionFactory() {
        try {
            ...
            // Solace JMS uses javax.jms.ConnectionFactory, but Spring Boot 3.x uses jakarta.jms.ConnectionFactory
            // Use Spring's CachingConnectionFactory which can handle javax.jms.ConnectionFactory
            // Cast the factory to javax.jms.ConnectionFactory first
            ...
            // Use reflection to call setTargetConnectionFactory with javax.jms.ConnectionFactory
            // because the method signature expects jakarta.jms.ConnectionFactory at compile time
            try {
                java.lang.reflect.Method setTargetMethod = org.springframework.jms.connection.CachingConnectionFactory.class
                    .getMethod("setTargetConnectionFactory", jakarta.jms.ConnectionFactory.class);
                // At runtime, we can pass the javax factory - the underlying implementation may accept it
                // If not, we'll need to create an adapter wrapper
                setTargetMethod.invoke(cachingFactory, (jakarta.jms.ConnectionFactory) javaxFactory);
            } catch (Exception e) {
                // If direct cast doesn't work, use reflection to bypass type checking
                try {
                    java.lang.reflect.Method setTargetMethod = org.springframework.jms.connection.CachingConnectionFactory.class
                        .getMethod("setTargetConnectionFactory", Object.class);
                    setTargetMethod.invoke(cachingFactory, javaxFactory);
                } catch (Exception e2) {
                    // Last resort: try to find any setTargetConnectionFactory method
                    java.lang.reflect.Method[] methods = org.springframework.jms.connection.CachingConnectionFactory.class
                        .getMethods();
                    for (java.lang.reflect.Method method : methods) {
                        if (method.getName().equals("setTargetConnectionFactory") && 
                            method.getParameterCount() == 1) {
                            method.invoke(cachingFactory, javaxFactory);
                            break;
                        }
                    }
                }
