...
public class CompressedLots {
    ...
    @JsonProperty("settlementDates")
    private List<String> settlementDates = new ArrayList<>(); // Settlement dates (nullable, falls back to trade date)
    
    @JsonProperty("settledQtys")
    private List<BigDecimal> settledQtys = new ArrayList<>(); // Settled quantities (nullable, defaults to originalQty)
    ...
    public List<TaxLot> inflate() {
        ...
        for (int i = 0; i < ids.size(); i++) {
            ...
            // Restore settlement date if available
            if (settlementDates != null && i < settlementDates.size() && settlementDates.get(i) != null && !settlementDates.get(i).isEmpty()) {
                builder.settlementDate(LocalDate.parse(settlementDates.get(i)));
            }
            
            // Restore settled quantity if available
            if (settledQtys != null && i < settledQtys.size() && settledQtys.get(i) != null) {
                builder.settledQuantity(settledQtys.get(i));
            }
    ...
    public static CompressedLots compress(List<TaxLot> lots) {
        ...
        for (TaxLot lot : lots) {
            ...
            // Store settlement date if available (nullable)
            compressed.getSettlementDates().add(lot.getSettlementDate() != null ? lot.getSettlementDate().toString() : null);
            // Store settled quantity if available (nullable)
            compressed.getSettledQtys().add(lot.getSettledQuantity());
        }
        return compressed;
    }
