...
~~1. **UPI History Table**: Track all UPI changes for audit purposes~~ ✅ **IMPLEMENTED**
~~2. **UPI Merge Detection**: Detect when backdated trades merge separate positions~~ ✅ **IMPLEMENTED**
~~3. **Notification**: Notify downstream systems when UPI changes due to backdated trades~~ ✅ **IMPLEMENTED**

### Implemented Features

All three future enhancements have been implemented:

1. **UPI History Table** (`upi_history` table):
   - Tracks all UPI changes with full audit trail
   - Records: CREATED, TERMINATED, REOPENED, INVALIDATED, MERGED, RESTORED
   - Includes triggering trade, backdated trade (if applicable), and reason

2. **UPI Merge Detection** (`UPIHistoryService.detectAndRecordMerge()`):
   - Automatically detects when backdated trades cause positions to merge
   - Records merge events with source position tracking
   - Integrated into coldpath recalculation flow

3. **Downstream Notifications** (`notifyDownstreamSystemsOfUPIChange()`):
   - Publishes `UPI_CHANGE_NOTIFICATION` events when UPI changes
   - Notifies RISK, P_AND_L, REPORTING, SETTLEMENT systems
   - Includes merge detection status and affected trade IDs

See `documents/UPI_HISTORY_AND_NOTIFICATIONS.md` for detailed documentation.
