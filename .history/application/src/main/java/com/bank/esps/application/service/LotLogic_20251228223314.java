...
public class LotLogic {
    ...
    public enum TaxLotMethod {
        ...
        HIFO   // Highest-In-First-Out (reduce from highest price first)
    }
    ...
    public LotAllocationResult reduceLots(PositionState state, BigDecimal qtyToReduce, 
        ...
        for (TaxLot lot : sortedLots) {
            ...
            if (closePrice != null && lot.getCostBasis() != null) {
                // Determine if this is a short position (negative quantity)
                boolean isShort = lotRemaining.compareTo(BigDecimal.ZERO) < 0;
                realizedPnL = lot.calculateRealizedPnL(closePrice, qtyToReduceFromLot, isShort);
            }
            ...
            log.debug("Reduced lot: lotId={}, reduced={}, remaining={}, realizedPnL={}", 
                    lot.getLotId(), qtyToReduceFromLot, newRemaining, realizedPnL);
    ...
    private List<TaxLot> sortLotsByMethod(List<TaxLot> lots, TaxLotMethod method) {
        ...
        } else if (method == TaxLotMethod.HIFO) {
            // HIFO: Highest price first (reduce from highest cost basis first)
            sorted.sort(Comparator
                    .comparing((TaxLot lot) -> lot.getCostBasis() != null ? lot.getCostBasis() : BigDecimal.ZERO, 
                            Comparator.reverseOrder())
                    .thenComparing((TaxLot lot) -> lot.getTradeDate() != null ? lot.getTradeDate() : LocalDate.MAX, 
                            Comparator.reverseOrder())
                    .thenComparing(lot -> lot.getLotId(), Comparator.reverseOrder()));
        }
