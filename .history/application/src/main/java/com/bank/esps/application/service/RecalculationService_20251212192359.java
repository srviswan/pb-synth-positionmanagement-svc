...
public class RecalculationService {
    ...
    public void recalculatePosition(TradeEvent backdatedTrade) {
        ...
        try {
            ...
            // This ensures all trades are reported to regulators, even backdated ones
            try {
                regulatorySubmissionService.submitTradeToRegulator(backdatedTrade, correctedSnapshot);
                log.info("Submitted backdated trade {} to regulator", backdatedTrade.getTradeId());
            } catch (Exception e) {
                // Log error but don't throw - regulatory submission failure shouldn't block recalculation
                log.error("Error submitting backdated trade {} to regulator: {}", 
                        backdatedTrade.getTradeId(), e.getMessage(), e);
            }
            ...
            if (previousUPI != null && newUPI != null && !previousUPI.equals(newUPI)) {
                log.warn("UPI changed from {} to {} for position {} - UPI-2 invalidated", 
                        previousUPI, newUPI, positionKey);
                
                // 10a. Record UPI invalidation in history
                upiHistoryService.recordUPIInvalidation(positionKey, previousUPI, newUPI, backdatedTrade,
                        String.format("Backdated trade %s caused UPI-2 (%s) to be invalidated and restored to UPI-1 (%s)",
                                backdatedTrade.getTradeId(), previousUPI, newUPI));
                
                // 10b. Detect UPI merge (if applicable)
                boolean mergeDetected = upiHistoryService.detectAndRecordMerge(positionKey, newUPI, backdatedTrade);
                
                // 10c. Find all trades associated with the invalidated UPI-2
                List<String> invalidatedTradeIds = findTradesForUPI(positionKey, previousUPI, replayedEvents);
                
                if (!invalidatedTradeIds.isEmpty()) {
                    // 10d. Publish regulatory event for UPI invalidation
                    publishUPIInvalidationEvent(positionKey, previousUPI, newUPI, invalidatedTradeIds, backdatedTrade);
                    
                    // 10e. Publish correction events for each affected trade
                    publishTradeCorrectionEvents(positionKey, previousUPI, newUPI, invalidatedTradeIds, 
                            replayedEvents, backdatedTrade);
                    
                    // 10f. Notify downstream systems of UPI change
                    notifyDownstreamSystemsOfUPIChange(positionKey, previousUPI, newUPI, invalidatedTradeIds, 
                            backdatedTrade, mergeDetected);
                }
            } else if (previousUPI != null && newUPI != null && previousUPI.equals(newUPI)) {
                // UPI didn't change, but check if position was restored (TERMINATED -> ACTIVE)
                if (currentSnapshot.getStatus() == com.bank.esps.domain.enums.PositionStatus.TERMINATED &&
                    correctedSnapshot.getStatus() == com.bank.esps.domain.enums.PositionStatus.ACTIVE) {
                    // Position was restored by backdated trade
                    upiHistoryService.recordUPIRestoration(positionKey, newUPI, previousUPI, backdatedTrade,
                            String.format("Backdated trade %s restored position from TERMINATED to ACTIVE",
                                    backdatedTrade.getTradeId()));
                }
            }
            
            // 11. Mark trade as processed
            idempotencyService.markAsProcessed(backdatedTrade, (long) replayedEvents.size());
            
            // 12. Publish correction event
    ...
    private ReplayResult replayEventsWithUPITracking(List<EventEntity> events) {
        ...
        for (EventEntity event : events) {
            try {
                ...
                // Get contract rules
                Contract contract = contractRulesService.getContract(
                ...
                switch (event.getEventType()) {
                    ...
                    case DECREASE -> {
                        LotAllocationResult result = lotLogic.reduceLots(state, tradeEvent.getQuantity(), contract, tradeEvent.getPrice());
    ...
    /**
     * Find all trades associated with a specific UPI
     * These are trades that occurred after the UPI was created and before it was invalidated
     */
    private List<String> findTradesForUPI(String positionKey, String targetUPI, List<EventEntity> replayedEvents) {
        List<String> tradeIds = new java.util.ArrayList<>();
        boolean foundUPIStart = false;
        
        for (EventEntity event : replayedEvents) {
            try {
                // Parse trade event from payload
                com.fasterxml.jackson.databind.JsonNode payload = objectMapper.readTree(event.getPayload());
                String tradeId = payload.has("tradeId") ? payload.get("tradeId").asText() : null;
                String tradeType = payload.has("tradeType") ? payload.get("tradeType").asText() : null;
                
                // Check if this is a NEW_TRADE that created the target UPI
                if ("NEW_TRADE".equals(tradeType) && tradeId != null && tradeId.equals(targetUPI)) {
                    foundUPIStart = true;
                    tradeIds.add(tradeId);
                    continue;
                }
                
                // If we found the UPI start, collect all subsequent trades until UPI changes
                if (foundUPIStart && tradeId != null) {
                    // Check if this trade would create a new UPI (NEW_TRADE on a TERMINATED position)
                    // This would indicate the end of UPI-2's lifecycle
                    if ("NEW_TRADE".equals(tradeType) && !tradeId.equals(targetUPI)) {
                        // This is a new UPI, so stop collecting
                        break;
                    }
                ...
                log.warn("Error parsing event payload for UPI tracking: {}", e.getMessage());
            }
        }
        
        log.info("Found {} trades associated with UPI {} for position {}", 
                tradeIds.size(), targetUPI, positionKey);
        return tradeIds;
    }
    
    /**
     * Publish regulatory event for UPI invalidation
     * Notifies regulator that trades with UPI-2 are invalid and need to be re-sent with UPI-1
     */
    private void publishUPIInvalidationEvent(
            String positionKey,
            String invalidatedUPI,
            String newUPI,
            List<String> invalidatedTradeIds,
            TradeEvent backdatedTrade) {
        
        try {
            // Create regulatory event payload
            String regulatoryEvent = String.format(
                    "{\"type\":\"UPI_INVALIDATION\"," +
                    "\"positionKey\":\"%s\"," +
                    "\"invalidatedUPI\":\"%s\"," +
                    "\"newUPI\":\"%s\"," +
                    "\"invalidatedTradeIds\":%s," +
                    "\"reason\":\"BACKDATED_TRADE_RECALCULATION\"," +
                    "\"backdatedTradeId\":\"%s\"," +
                    "\"effectiveDate\":\"%s\"," +
                    "\"occurredAt\":\"%s\"," +
                    "\"actionRequired\":\"RESUBMIT_TRADES_WITH_NEW_UPI\"," +
                    "\"message\":\"Trades submitted with UPI %s are invalid. These trades must be re-submitted with UPI %s.\"}",
                    positionKey,
                    invalidatedUPI,
                    newUPI,
                    objectMapper.writeValueAsString(invalidatedTradeIds),
                    backdatedTrade.getTradeId(),
            ...
            // Publish to regulatory events topic
            messageProducer.publishRegulatoryEvent(positionKey, regulatoryEvent);
            
            log.warn("Published UPI invalidation regulatory event for position {}: UPI {} -> {}, {} trades affected", 
                    positionKey, invalidatedUPI, newUPI, invalidatedTradeIds.size());
            
        } catch (Exception e) {
            log.error("Error publishing UPI invalidation regulatory event for position {}", 
                    positionKey, e);
        }
    }
    
    /**
     * Publish correction events for each trade affected by UPI invalidation
     * These correction events notify the regulator that specific trades need to be corrected
     */
    private void publishTradeCorrectionEvents(
            String positionKey,
            String invalidatedUPI,
            String newUPI,
            List<String> invalidatedTradeIds,
            List<EventEntity> replayedEvents,
            TradeEvent backdatedTrade) {
        
        try {
            log.info("Publishing correction events for {} trades affected by UPI invalidation", 
                    invalidatedTradeIds.size());
            
            for (String tradeId : invalidatedTradeIds) {
                try {
                    // Find the event for this trade
                    EventEntity tradeEvent = replayedEvents.stream()
                            .filter(event -> {
                                try {
                                    com.fasterxml.jackson.databind.JsonNode payload = 
                                            objectMapper.readTree(event.getPayload());
                                    String eventTradeId = payload.has("tradeId") ? 
                                            payload.get("tradeId").asText() : null;
                                    return tradeId.equals(eventTradeId);
                                } catch (Exception e) {
                                    return false;
                                }
                            })
                    ...
                    if (tradeEvent == null) {
                        log.warn("Could not find event for trade {} in replayed events", tradeId);
                        continue;
                    }
                    
                    // Parse trade details from event payload
                    com.fasterxml.jackson.databind.JsonNode payload = 
                            objectMapper.readTree(tradeEvent.getPayload());
                    
                    // Create correction event for this trade
                    String correctionEvent = String.format(
                            ...
                            "\"message\":\"Trade %s was submitted with UPI %s which is now invalid. This trade must be corrected with UPI %s.\"}",
                            tradeId,
                            positionKey,
                            invalidatedUPI,
                            newUPI,
                            payload.has("tradeType") ? payload.get("tradeType").asText() : "UNKNOWN",
                            payload.has("quantity") ? payload.get("quantity").asText() : "0",
                            payload.has("price") ? payload.get("price").asText() : "0",
                            tradeEvent.getEffectiveDate(),
                            backdatedTrade.getTradeId(),
                            java.time.OffsetDateTime.now(),
                            tradeId,
                    ...
                    messageProducer.publishRegulatoryEvent(positionKey, correctionEvent);
                    
                    log.info("Published correction event for trade {} (UPI {} -> {})", 
                            tradeId, invalidatedUPI, newUPI);
                    
                } catch (Exception e) {
                    log.error("Error publishing correction event for trade {}", tradeId, e);
                    // Continue with other trades
                }
            }
            
            log.info("Completed publishing correction events for {} trades", invalidatedTradeIds.size());
            
        } catch (Exception e) {
            log.error("Error publishing trade correction events for position {}", positionKey, e);
        }
    }
    
    /**
     * Notify downstream systems when UPI changes due to backdated trades
     * This includes risk systems, P&L systems, reporting systems, etc.
     */
    private void notifyDownstreamSystemsOfUPIChange(
            String positionKey,
            String previousUPI,
            String newUPI,
            List<String> invalidatedTradeIds,
            TradeEvent backdatedTrade,
            boolean mergeDetected) {
        
        try {
            // Create downstream notification event
            String downstreamEvent = String.format(
                    ...
                    "\"message\":\"UPI changed from %s to %s due to backdated trade %s. All downstream systems must update position references.\"," +
            ...
            // Publish to downstream notifications topic
            messageProducer.publishCorrectionEvent(positionKey, downstreamEvent);
            
            log.info("Published downstream notification for UPI change: position={}, UPI={} -> {}, merge={}, {} trades affected", 
                    positionKey, previousUPI, newUPI, mergeDetected, invalidatedTradeIds.size());
            
        } catch (Exception e) {
            log.error("Error publishing downstream notification for UPI change on position {}", positionKey, e);
        }
    }
