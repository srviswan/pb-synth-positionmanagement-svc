...
public class TradeValidationService {
    ...
    public ValidationResult validate(TradeEvent trade) {
        ...
        // Generate position key if not provided but account/instrument/currency are available
        if ((trade.getPositionKey() == null || trade.getPositionKey().isEmpty()) &&
            trade.getAccount() != null && !trade.getAccount().isEmpty() &&
            trade.getInstrument() != null && !trade.getInstrument().isEmpty() &&
            trade.getCurrency() != null && !trade.getCurrency().isEmpty()) {
            // Generate position key from account+instrument+currency
            // Determine if short based on quantity sign (negative = short)
            boolean isShort = trade.getQuantity() != null && trade.getQuantity().compareTo(BigDecimal.ZERO) < 0;
            String generatedKey = positionKeyGenerator.generatePositionKey(
                trade.getAccount(), 
                trade.getInstrument(), 
                trade.getCurrency(), 
                isShort
            );
            trade.setPositionKey(generatedKey);
            log.debug("Generated position key {} for trade {} from account={}, instrument={}, currency={}, isShort={}", 
                    generatedKey, trade.getTradeId(), trade.getAccount(), trade.getInstrument(), 
                    trade.getCurrency(), isShort);
        }
        
        // Validate position key is now available (either provided or generated)
        if (trade.getPositionKey() == null || trade.getPositionKey().isEmpty()) {
            if (trade.getAccount() == null || trade.getAccount().isEmpty() ||
                trade.getInstrument() == null || trade.getInstrument().isEmpty() ||
                trade.getCurrency() == null || trade.getCurrency().isEmpty()) {
                errors.add("Position key is required, or provide account, instrument, and currency to generate it");
            } else {
                errors.add("Position key generation failed");
            }
        }
