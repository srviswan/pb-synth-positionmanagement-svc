...
public class HotpathPositionService {
    ...
    public void processCurrentDatedTrade(TradeEvent tradeEvent) {
        ...
        SnapshotEntity snapshot = snapshotRepository.findById(positionKey)
                .orElseGet(() -> {
                    SnapshotEntity newSnapshot = createNewSnapshot(positionKey, tradeEvent);
        ...
        log.debug("Processing trade {} for position {}, current state has {} lots, total qty: {}", 
                tradeEvent.getTradeId(), positionKey, state.getOpenLots().size(), state.getTotalQty());
    ...
    private PositionState inflateSnapshot(SnapshotEntity snapshot) {
        try {
            ...
            // Set the raw list - getOpenLots() will filter closed lots
            List<TaxLot> inflatedLots = compressed.inflate();
            state.setOpenLots(inflatedLots);
            log.debug("Inflated {} lots from snapshot for position {}", inflatedLots.size(), snapshot.getPositionKey());
            return state;
        } catch (Exception e) {
