        ...
        if (signChanged) {
            ...
            try {
                upiHistoryService.recordUPIChange(
                    ...
                    "CREATED",
                    tradeEvent,
                    String.format("New position created after sign change: %s (trade %s)", 
                            isLongAfter ? "LONG" : "SHORT",
                            tradeEvent.getTradeId()));
            } catch (Exception e) {
                log.error("Error recording UPI creation for sign change on position {}", positionKey, e);
            }
            
            log.info("✅ Position {} transitioned: Old UPI {} (TERMINATED) -> New UPI {} (ACTIVE, {})", 
                    positionKey, oldUPI, newUPI, isLongAfter ? "LONG" : "SHORT");
        } else {
