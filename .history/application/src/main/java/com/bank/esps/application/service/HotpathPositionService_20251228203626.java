...
public class HotpathPositionService {
    ...
    public void processCurrentDatedTrade(TradeEvent tradeEvent) {
        ...
        if (tradeEvent.isIncrease()) {
            ...
            // Extract settlement date using hybrid approach:
            // 1. Use settlementDate from TradeEvent if provided
            // 2. Fallback to effectiveDate (trade date)
            LocalDate settlementDate = tradeEvent.getSettlementDate() != null ? 
                    tradeEvent.getSettlementDate() : tradeEvent.getEffectiveDate();
            
            result = lotLogic.addLot(state, tradeEvent.getQuantity(), 
                    tradeEvent.getPrice(), tradeEvent.getEffectiveDate(),
                    settlementDate, tradeEvent.getQuantity()); // settledQuantity defaults to quantity
            log.debug("After adding lot, state now has {} open lots ({} total), total qty: {}, settlementDate: {}", 
                    state.getOpenLots().size(), state.getAllLots().size(), state.getTotalQty(), settlementDate);
        } else {
            ...
            if (result.getRemainingQuantity() != null && result.getRemainingQuantity().compareTo(BigDecimal.ZERO) < 0) {
                ...
                lotLogic.addLot(state, shortQty, tradeEvent.getPrice(), tradeEvent.getEffectiveDate(),
                        settlementDate, shortQty.abs()); // settledQuantity is absolute value for short
        ...
        if (signChanged) {
            ...
            // Extract settlement date for new position after sign change
            LocalDate settlementDate = tradeEvent.getSettlementDate() != null ? 
                    tradeEvent.getSettlementDate() : tradeEvent.getEffectiveDate();
            lotLogic.addLot(newState, totalQtyAfterTrade, tradeEvent.getPrice(), tradeEvent.getEffectiveDate(),
                    settlementDate, totalQtyAfterTrade.abs()); // settledQuantity is absolute value
