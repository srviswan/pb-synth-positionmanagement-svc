...
public class HotpathPositionService {
    ...
    public void processCurrentDatedTrade(TradeEvent tradeEvent) {
        // Clear persistence context at start to ensure fresh data
        ...
        log.debug("Successfully saved snapshot for position {}: version {}, compressed data length: {}", 
    ...
    private PositionState inflateSnapshot(SnapshotEntity snapshot) {
        try {
            ...
            com.fasterxml.jackson.databind.JsonNode jsonNode = deserializingMapper.readTree(compressedJson);
            ...
            log.info("Successfully inflated {} lots from manually constructed CompressedLots", inflatedLots.size());
            ...
            BigDecimal totalQty = state.getTotalQty();
            log.info("Successfully inflated {} lots from snapshot for position {}: {} open lots, total qty: {}", 
            ...
            log.error("CRITICAL: Error inflating snapshot for position {}: {}. Compressed data: {}", 
