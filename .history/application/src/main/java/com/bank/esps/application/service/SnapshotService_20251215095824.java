...
public class SnapshotService {
    ...
    public SnapshotEntity createNewSnapshot(String positionKey, TradeEvent tradeEvent) {
        ...
        // Set lookup fields for efficient querying
        snapshot.setAccount(tradeEvent.getAccount());
        snapshot.setInstrument(tradeEvent.getInstrument());
        snapshot.setCurrency(tradeEvent.getCurrency());
        snapshot.setContractId(tradeEvent.getContractId());
        
        // Initialize PriceQuantity schedule with first trade
        PriceQuantitySchedule schedule = PriceQuantitySchedule.builder()
                .unit("SHARES")
                .currency(tradeEvent.getCurrency() != null ? tradeEvent.getCurrency() : "USD")
                .build();
    ...
    public void updateSnapshot(SnapshotEntity snapshot, PositionState state, 
        ...
        // Update lookup fields from trade event (if provided)
        if (tradeEvent != null) {
            if (tradeEvent.getAccount() != null) {
                snapshot.setAccount(tradeEvent.getAccount());
            }
            if (tradeEvent.getInstrument() != null) {
                snapshot.setInstrument(tradeEvent.getInstrument());
            }
            if (tradeEvent.getCurrency() != null) {
                snapshot.setCurrency(tradeEvent.getCurrency());
            }
            if (tradeEvent.getContractId() != null) {
                snapshot.setContractId(tradeEvent.getContractId());
            }
        }
