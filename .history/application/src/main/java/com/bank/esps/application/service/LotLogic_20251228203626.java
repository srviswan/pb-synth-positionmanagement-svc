...
public class LotLogic {
     ...
     * @param settlementDate Settlement date (when interest accrual starts) - nullable, falls back to tradeDate
     * @param settledQuantity Quantity that was actually settled - nullable, defaults to qty
     */
    public LotAllocationResult addLot(PositionState state, BigDecimal qty, BigDecimal price, LocalDate tradeDate, 
                                     LocalDate settlementDate, BigDecimal settledQuantity) {
        String lotId = generateLotId();
        TaxLot newLot = TaxLot.builder()
                .id(lotId)
                .tradeDate(tradeDate)
                .settlementDate(settlementDate) // Settlement date for interest accrual
                ...
                .settledQuantity(settledQuantity != null ? settledQuantity : qty) // Default to qty if not specified
                .build();
        ...
        log.info("✅ Added new lot: {} with qty: {}, price: {}, tradeDate: {}, settlementDate: {}, remaining notional: {}", 
                lotId, qty, price, tradeDate, settlementDate != null ? settlementDate : tradeDate, notional);
        return result;
    }
    
    /**
     * Add a new lot to the position (backward compatibility - uses tradeDate as settlementDate)
     */
    public LotAllocationResult addLot(PositionState state, BigDecimal qty, BigDecimal price, LocalDate tradeDate) {
        return addLot(state, qty, price, tradeDate, null, null);
    }
