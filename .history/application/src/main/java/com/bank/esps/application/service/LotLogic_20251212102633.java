...
public class LotLogic {
    ...
    public LotAllocationResult reduceLots(PositionState state, BigDecimal qtyToReduce, ContractRules rules) {
        ...
        LotAllocationResult result = new LotAllocationResult();
        // Get all lots (including closed ones for filtering)
        List<TaxLot> allLots = state.getOpenLots(); // This filters closed lots
        List<TaxLot> openLots = allLots.stream()
                .filter(lot -> lot.getRemainingQty().compareTo(BigDecimal.ZERO) > 0)
