...
public class HotpathPositionService {
    ...
    private SnapshotEntity createNewSnapshot(String positionKey, TradeEvent tradeEvent) {
        CompressedLots emptyLots = new CompressedLots();
        PositionState initialState = new PositionState();
        
        return SnapshotEntity.builder()
                .positionKey(positionKey)
                .lastVer(0L)
                .uti(tradeEvent.getTradeId()) // Initial UTI
                .status(com.bank.esps.domain.enums.PositionStatus.ACTIVE)
                .reconciliationStatus(ReconciliationStatus.RECONCILED)
                .taxLotsCompressed(toJson(CompressedLots.compress(emptyLots.inflate())))
                .summaryMetrics(toJson(createSummaryMetrics(initialState)))
                .version(0L)
                .build();
    }
    ...
    private EventEntity createEventEntity(TradeEvent trade, long version, LotAllocationResult result) {
        try {
            return EventEntity.builder()
                    ...
                    .eventVer(version)
                    .eventType(EventType.valueOf(trade.getTradeType()))
                    .effectiveDate(trade.getEffectiveDate())
                    .occurredAt(OffsetDateTime.now())
                    .payload(toJson(trade))
                    .metaLots(toJson(result.getAllocationsMap()))
