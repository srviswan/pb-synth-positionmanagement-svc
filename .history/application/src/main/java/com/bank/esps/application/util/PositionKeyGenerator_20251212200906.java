...
public class PositionKeyGenerator {
    ...
    public int getPartitionNumber(String positionKey, int partitionCount) {
        ...
        // Use simple hash code modulo (matches PostgreSQL hash partitioning behavior)
        // Note: This is a simplified version. PostgreSQL uses its own hash function.
        // For exact alignment, you'd need to use the same hash function as PostgreSQL.
        int hashCode = positionKey.hashCode();
        int partition = Math.abs(hashCode) % partitionCount;
        
        log.debug("Position key {} maps to partition {}", positionKey, partition);
        return partition;
    }
