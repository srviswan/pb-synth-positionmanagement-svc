...
public class HotpathPositionService {
    ...
    public void processCurrentDatedTrade(TradeEvent tradeEvent) {
        ...
        try {
            savedEvent = eventStoreRepository.save(event);
        ...
        try {
            if (entityManager != null) {
                ...
                Optional<EventEntity> verifyEvent = eventStoreRepository.findById(
    ...
    private SnapshotEntity createNewSnapshot(String positionKey, TradeEvent tradeEvent) {
        ...
        com.bank.esps.domain.model.PriceQuantitySchedule schedule = 
            com.bank.esps.domain.model.PriceQuantitySchedule.builder()
                .unit("SHARES")
                .currency("USD")
                .build();
    ...
    private void updateSnapshot(SnapshotEntity snapshot, PositionState state, 
        ...
        // Update PriceQuantity schedule with current state
     ...
     * Tracks quantity and price over time (CDM-inspired)
     */
    private void updatePriceQuantitySchedule(SnapshotEntity snapshot, PositionState state, TradeEvent tradeEvent) {
        log.info("🔄 ENTERING updatePriceQuantitySchedule for position {}", snapshot.getPositionKey());
        try {
            ...
            if (currentQty.compareTo(BigDecimal.ZERO) > 0) {
                // Calculate weighted average price from open lots
                ...
                // If qty is zero, use the trade price (for closure tracking)
                currentPrice = tradeEvent.getPrice();
            }
            
            // Add/update schedule entry for this trade's effective date
            schedule.addOrUpdate(tradeEvent.getEffectiveDate(), currentQty, currentPrice);
            
            snapshot.setPriceQuantitySchedule(toJson(schedule));
            
            log.info("✅ Updated PriceQuantity schedule for position {}: {} entries, current qty: {}, current price: {}", 
                    snapshot.getPositionKey(), schedule.getSchedule().size(), currentQty, currentPrice);
        } catch (Exception e) {
            log.warn("Error updating PriceQuantity schedule for position {}: {}", 
                    snapshot.getPositionKey(), e.getMessage());
            // Don't fail snapshot update if schedule update fails
        }
    }
