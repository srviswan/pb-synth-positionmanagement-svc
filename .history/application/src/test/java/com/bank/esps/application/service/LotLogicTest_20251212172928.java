...
class LotLogicTest {
    ...
    void testReduceLotsFIFO() {
        PositionState state = new PositionState();
        ContractRules rules = ContractRules.builder()
                .contractId("C1")
        ...
        LotAllocationResult result = lotLogic.reduceLots(state, new BigDecimal("150"), rules, new BigDecimal("55.00"));
    ...
    void testReduceLotsLIFO() {
        ...
        LotAllocationResult result = lotLogic.reduceLots(state, new BigDecimal("100"), rules, new BigDecimal("55.00"));
        
        assertEquals(1, result.getAllocations().size());
        assertEquals(new BigDecimal("100"), result.getAllocations().get(0).getQty());
        assertEquals(new BigDecimal("60.00"), result.getAllocations().get(0).getPrice());
    ...
    void testReduceLotsInsufficientQuantity() {
        ...
        assertThrows(IllegalStateException.class, () -> {
            lotLogic.reduceLots(state, new BigDecimal("200"), rules, new BigDecimal("55.00"));
        });
    }
}
