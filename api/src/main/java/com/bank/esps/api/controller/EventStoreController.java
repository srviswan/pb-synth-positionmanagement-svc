...
public class EventStoreController {
    ...
    private final com.bank.esps.application.service.RecalculationService recalculationService;
    private final ObjectMapper objectMapper;
    
    public EventStoreController(
            EventStoreRepository eventStoreRepository,
            SnapshotRepository snapshotRepository,
            HotpathPositionService hotpathPositionService,
            com.bank.esps.application.service.RecalculationService recalculationService,
    ...
    public ResponseEntity<Map<String, Object>> getEvent(
        ...
        try {
            ...
            if (event.isPresent()) {
                ...
                eventMap.put("positionKey", e.getPositionKey());
                eventMap.put("version", e.getEventVer());
                eventMap.put("type", e.getEventType().name());
                eventMap.put("effectiveDate", e.getEffectiveDate().toString());
                eventMap.put("occurredAt", e.getOccurredAt().toString());
                eventMap.put("payloadLength", e.getPayload() != null ? e.getPayload().length() : 0);
                
                // Include metaLots if present
                if (e.getMetaLots() != null && !e.getMetaLots().trim().isEmpty()) {
                    try {
                        com.fasterxml.jackson.databind.ObjectMapper mapper = 
                            new com.fasterxml.jackson.databind.ObjectMapper();
                        Map<String, Object> metaLots = mapper.readValue(e.getMetaLots(), Map.class);
                        eventMap.put("metaLots", metaLots);
                    } catch (Exception ex) {
                        eventMap.put("metaLots", e.getMetaLots());
                        eventMap.put("metaLotsParseError", ex.getMessage());
                    }
                }
    ...
    /**
     * Manually trigger recalculation for a backdated trade (for testing)
     */
    @PostMapping("/recalculate")
    public ResponseEntity<Map<String, Object>> triggerRecalculation(@RequestBody com.bank.esps.domain.event.TradeEvent tradeEvent) {
        Map<String, Object> response = new HashMap<>();
        try {
            // Ensure sequenceStatus is set to BACKDATED
            if (tradeEvent.getSequenceStatus() == null) {
                tradeEvent.setSequenceStatus(com.bank.esps.domain.enums.TradeSequenceStatus.BACKDATED);
            }
            
            recalculationService.recalculatePosition(tradeEvent);
            
            response.put("status", "success");
            response.put("message", "Recalculation triggered successfully");
